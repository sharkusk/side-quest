...
**Date:** 2026-07-04
**Quests:** [[SQ-0064]] (onboard as the single, plugin-aware front door), [[SQ-0065]] (expose the plugin's binary on the user's PATH — `install-cli`/`uninstall-cli` + launcher), [[SQ-0066]] (agent-guidance lifecycle — `SessionStart` hook + first-run consent offers).
...
**D1 — Command surface: option C.** A dedicated, machine-global `side-quest install-cli` / `side-quest uninstall-cli` pair owns the PATH mechanism. `onboard` becomes plugin-aware (D6). The CLI-enable is offered by the agent (D5), not silently forced.
...
- **launcher installed by `install-cli`** — a pure, **read-only** resolver of the data-dir binary (D4). Simpler than the plugin shim: **no download/checksum path** (provisioning is the hook's job). Ships as a small `side-quest` shell script plus a `side-quest.cmd` for Windows.
...
First session after install, the `SessionStart` sentinel is absent → the agent offers to put `side-quest` on your PATH. On yes it runs `side-quest install-cli` (D7): writes `~/.local/bin/side-quest` (+ `.cmd` on Windows), reports the path, or tells you to add a dir to PATH. Now `side-quest …` works in your terminal, and your **own** terminal commits fire the hooks. Declining is remembered.
...
`/plugin marketplace update side-quest`. `SessionStart` sees the bundled VERSION changed and re-provisions the new binary into the data dir. The CLI launcher is self-healing (D4) → it resolves the new version automatically; **no re-run needed**. Old cache dirs are Claude's to clean.
...
- Unit: launcher resolution order (data-dir hit → exec / data-dir-empty → "finish setup" notice, **no download** / data-dir-absent → self-removal warn); marker detection in `uninstall-cli`; `install-cli` PATH-dir selection given a synthetic `$PATH`; `onboard` plugin-detection branch (skips `.mcp.json` when `CLAUDE_PLUGIN_DATA`/plugin path present).
